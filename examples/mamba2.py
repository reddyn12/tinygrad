...
class InferenceCache(NamedTuple):
  ...
  @staticmethod
  def alloc(batch_size: int, args: Mamba2Config):
...
class MambaBlock:
   def __init__(self, args: Mamba2Config, RMS_eps=1e-5):
      self.mixer = Mamba2(args)
...
class Mamba2LMHeadModel:
  def __init__(self, args: Mamba2Config):
    ...
    #                     dict(
    #                         mixer=Mamba2(args),
    #                         norm=nn.RMSNorm(args.d_model, 0.00001),
    #                     )
    #                 )
    #                 for _ in range(args.n_layer)
    #             ]
    #         ),
    #         norm_f=nn.RMSNorm(args.d_model, 0.00001),
    #     )
    # )
    self.embedding=nn.Embedding(args.vocab_size, args.d_model)
    self.layers = []
    for _ in range(args.n_layer):
      #  self.layers.append(Mamba2(args))
      #  self.layers.append(nn.RMSNorm(args.d_model, 0.00001))
      self.layers.append(MambaBlock(args, 0.00001))
    self.norm_f = RMSNorm(args.d_model)

    self.lm_head = nn.Linear(
        args.d_model, args.vocab_size, bias=False
    )
  ...
  def from_pretrained(huggingface_model_id: str):
      ...
      # )
      state_dict = torch_load(fetch(state_dict_path))
      replace_dict = {}
      for k,v in state_dict.items():
        nk = k
        nk = nk.replace('backbone.', '')
        # nk = nk.replace('mixer.', '')
        replace_dict[nk] = v
          
      model = Mamba2LMHeadModel(args)
      # model.load_state_dict(state_dict)
      load_state_dict(model, replace_dict)
      return model

  def __call__(
      self, input_ids: Tensor, h: list[InferenceCache] | list[None] | None = None
...
class Mamba2:
  def __init__(self, args: Mamba2Config):
    ...
    self.dt_bias = Tensor.empty(args.nheads)
    self.A_log = Tensor.empty(args.nheads)
    self.D = Tensor.empty(args.nheads)
  ...
  def __call__(self, u: Tensor, h: InferenceCache | None = None):
    ...
    A = -self.A_log.exp()  # (nheads,)
    zxbcdt = self.in_proj(u)  # (batch, seqlen, d_in_proj)
    z, xBC, dt = Tensor.split(
        zxbcdt,
        [
            self.args.d_inner,
    ...
    dt = (dt + self.dt_bias).softplus()  # (batch, seqlen, nheads)

    # Pad or truncate xBC seqlen to d_conv
    conv_state = Tensor.pad(
        Tensor.rearrange(xBC, "b l d -> b d l"), (self.args.d_conv - u.shape[1], 0)
    )
    ...
    x = Tensor.rearrange(x, "b l (h p) -> b l h p", p=self.args.headdim)
    y, ssm_state = ssd(
        x * dt.unsqueeze(-1),
        A * dt,
        Tensor.rearrange(B, "b l n -> b l 1 n"),
        Tensor.rearrange(C, "b l n -> b l 1 n"),
  ...
  def step(self, u: Tensor, h: InferenceCache) -> tuple[Tensor, InferenceCache]:
    ...
    # SHOULD BE EQUIVAENT TO:
    h.conv_state.assign(h.conv_state[:, :, 1:].cat(xBC.unsqueeze(-1), dim=-1).realize())
    # h.conv_state.copy_(torch.roll(h.conv_state, shifts=-1, dims=-1))
    # h.conv_state[:, :, -1] = xBC
    # Convolution step
    xBC = Tensor.sum(
        h.conv_state * Tensor.rearrange(self.conv1d.weight, "d 1 w -> d w"), axis=-1
    )
    ...
    A = -Tensor.exp(self.A_log)  # (nheads,)

    # SSM step
    dt = Tensor.softplus(dt + self.dt_bias)  # (batch, nheads)
    dA = Tensor.exp(dt * A)  # (batch, nheads)
    x = Tensor.rearrange(x, "b (h p) -> b h p", p=self.args.headdim)
    dBx = Tensor.einsum("bh, bn, bhp -> bhpn", dt, B, x)
    # h.ssm_state.copy_(h.ssm_state * Tensor.rearrange(dA, "b h -> b h 1 1") + dBx)
    h.ssm_state.assign(h.ssm_state * Tensor.rearrange(dA, "b h -> b h 1 1") + dBx)

    y = Tensor.einsum("bhpn, bn -> bhp", h.ssm_state, C)
...
def segsum(x: Tensor) -> Tensor:
  ...
  mask = Tensor.tril(Tensor.ones(T, T, dtype=dtypes.bool), diagonal=-1)
  x = x.masked_fill(~mask, 0)
  x_segsum = Tensor.cumsum(x, dim=-2)
  mask = Tensor.tril(Tensor.ones(T, T, dtype=dtypes.bool), diagonal=0)
...
def ssd(x, A, B, C, chunk_size, initial_states=None):
  ...
  A_cumsum = Tensor.cumsum(A, dim=-1)

  # 1. Compute the output for each intra-chunk (diagonal blocks)
  L = Tensor.exp(segsum(A))
  ...
  decay_chunk = Tensor.exp(segsum(Tensor.pad(A_cumsum[:, :, :, -1], (1, 0))))
...
class RMSNorm:
  def __init__(self, d: int, eps: float = 1e-5):
    ...
    """
    self.eps = eps
    self.weight = Tensor.ones(d)

  def __call__(self, x, z=None):
    if z is not None:
      x = x * z.silu()
    return x * Tensor.rsqrt(x.pow(2).mean(-1, keepdim=True) + self.eps) * self.weight

if __name__ == "__main__":
  import time
  from transformers import AutoTokenizer
  # model = Mamba2LMHeadModel(Mamba2Config(512))
  # print(get_state_dict(model).keys())

  model = Mamba2LMHeadModel.from_pretrained("state-spaces/mamba2-1.3b")
  # print(get_state_dict(model).keys())
  ...
  def generate(prompt: str, seed: int = 0, show_perf: bool = True):
      ...
      input_ids = Tensor(tokenizer(prompt, return_tensors='np').input_ids[0])
