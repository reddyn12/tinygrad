# https://github.com/state-spaces/mamba/tree/main/mamba_ssm/modules
...
MODELS = {
  "130m": {"dim":  768, "n_layers": 24, "vocab_size": 50277, "pad_vocab_size_multiple": 16},
  "370m": {"dim": 1024, "n_layers": 48, "vocab_size": 50277, "pad_vocab_size_multiple": 16},
  "790m": {"dim": 1536, "n_layers": 48, "vocab_size": 50277, "pad_vocab_size_multiple": 16},
  "1.4b": {"dim": 2048, "n_layers": 48, "vocab_size": 50277, "pad_vocab_size_multiple": 16},
  "2.8b": {"dim": 2560, "n_layers": 64, "vocab_size": 50277, "pad_vocab_size_multiple": 16},
}

def fetch_weights(model_name: str) -> Dict[str, Tensor]:
  if model_name not in MODELS:
    raise ValueError(f"Requested unknown mamba model: {model_name}")
  downloaded = fetch(f"https://huggingface.co/state-spaces/mamba2-{model_name}/resolve/main/pytorch_model.bin?download=true")
...
class MambaMixer:
  def __init__(
    ...
    self.in_proj = nn.Linear(self.dim, d_in_proj, bias=bias)

    conv_dim = self.d_inner + 2 * self.d_state
    self.conv1d = nn.Conv1d(in_channels=conv_dim, out_channels=conv_dim, bias=conv_bias,
                            kernel_size=d_conv, groups=conv_dim, padding=d_conv-1)

    # self.out_proj = nn.Linear(self.d_inner, self.dt_rank + self.d_state * 2, bias=False)

    dt = Tensor.uniform(self.d_inner, low=math.log(dt_min), high=math.log(dt_max)).exp().maximum(dt_init_floor)
    inv_dt = dt + (1 - (-dt).exp()).log()
    self.dt_bias = Tensor.ones(self.nheads)
    # S4D real initialization
    self.A_log = Tensor.ones(self.nheads).log()

    # D "skip" parameter
    self.D = Tensor.ones(self.nheads)  # Keep in fp32
